 ...
 * @param Idx   : 从0开始
 *   对于网口，该参数为“IP+端口”在字符串，如"192.168.1.10:4567"。如果省略端口，则默认为9100。
 * @return 操作是否成功
 */
TX_API BOOL WINAPI TxOpenPrinter(DWORD Type, DWORD_PTR Idx);
 ...
 * @return 操作是否成功
 */
TX_API BOOL WINAPI TxWritePrinter(const char* buf, DWORD len);
 ...
 * @return 返回实际的字节数
 */
TX_API DWORD WINAPI TxReadPrinter(char* buf, DWORD len);
 ...
 */
TX_API DWORD WINAPI TxGetStatus();

/**
 * @brief 与TxGetStatus类似，区别是USB接口通过指令查询状态。
 * @return 与TxGetStatus相同
 */
TX_API DWORD WINAPI TxGetStatus2();

/**
 * @brief 关闭连接
 */
TX_API void WINAPI TxClosePrinter();

/**
 * @brief 发送初始化指令
 */
TX_API void WINAPI TxInit();

/**
 * @brief 输出字符串（以\0结束）
 * @param str  : 字符串
 */
TX_API void WINAPI TxOutputString(const char* str);

/**
 * @brief 输出回车、换行
 */
TX_API void WINAPI TxNewline();

/**
 * @brief 输出字符串（以\0结束），并自动添加回车、换行
 * @param str  : 字符串
 */
TX_API void WINAPI TxOutputStringLn(const char* str);
 ...
 */
TX_API void WINAPI TxDoFunction(DWORD func, int param1, int param2);

/**
 * @brief 恢复字体效果（大小、粗体等）为原始状态。
 */
TX_API void WINAPI TxResetFont();
 ...
 * @return 操作是否成功
 */
TX_API BOOL WINAPI TxPrintBarcode(DWORD type, char* str);
 ...
 * @return 操作是否成功
 */
TX_API BOOL WINAPI TxPrintImage(const char* path);

/**
 * @brief 设置串口参数
 * @param attr  : 属性组合
 */
TX_API void WINAPI TxSetupSerial(DWORD attr);
 ...
 * @param len  : 内容长度
 */
TX_API void WINAPI TxPrintQRcode(const char *data, WORD len);
